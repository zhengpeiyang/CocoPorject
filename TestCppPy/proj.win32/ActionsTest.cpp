...
void ActionsDemo::onEnter()
{
	...
	{
		CCLabelTTF* ll = CCLabelTTF::create(strSubTitle.c_str(),"Thonburi",22);
		addChild(ll,1);
		ll->setPosition(ccp(VisibleRect::center().x,VisibleRect::top().y-60));
	}
...
void ActionsDemo::centerSprites(unsigned int numberOfSprites)
{
	...
	else if(numberOfSprites == 2)
	{
		...
		m_tamara->setPosition(ccp(s.width*2/3,s.height/2));
		m_grossini->setVisible(false);
	}
	else if(numberOfSprites == 3)
	{
		...
		m_kathia->setPosition(ccp(s.width/4*3,s.height/2));
	}
...
void ActionsDemo::alignSpritesLeft(unsigned int numberOfSprites)
{
	...
	else if(numberOfSprites == 2)
	{
		...
		m_tamara->setPosition(ccp(60,s.height*2/3));
		m_grossini->setVisible(false);
	}
	else if(numberOfSprites == 3)
	{
...
void ActionMove::onEnter()
{
	...
	CCActionInterval* actionBack = actionBy->reverse();
 	...
 	m_grossini->runAction(CCSequence::create(actionBy,actionBack,NULL));
